...
const GLchar* vertexShaderSource { 
    ...
    "out vec4 myVarVertexColor; // specify a color output to the fragment shader\n"
    "void main()\n"
    "{\n"
    "   gl_Position = vec4(aPos.x, aPos.y, aPos.z, 1.0);\n"
    "   myVarVertexColor = vec4(0.5f, 0.0f, 0.0f, 1.0f);\n"  
    "}\0"
};
...
const GLchar* fragmentShaderSource {
    "#version 330 core\n"
    "out vec4 FragColor;\n"
    "uniform vec4 myVarVertexColor;\n"
    "void main()\n"
    "{\n"
    "    FragColor = myVarVertexColor;\n"
    "}\0"
};
...
int main() 
{
    ...
    // Vertex shader input = vertex attribute
    // Max nº of attributes:
    int nrAttributes;
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // We must use it first if we want to update the uniform
        // The updates apply to the uniform of the currently active program
        glUseProgram(shaderProgram);

        // Get the location of our uniform 
        // (uniforms are global so we can pass the shader program, not the shader where the uniform is declared)
        GLint vertexColorLocation { glGetUniformLocation(shaderProgram, "myVarVertexColor")};

        // Change the color over time
        float greenValue{ sin((float) glfwGetTime()) / 2.0f + 0.5f};
        glUniform4f(vertexColorLocation, 0.0f, greenValue, 0.0f, 1.0f);

        // Continue with the rendering of the triangle
