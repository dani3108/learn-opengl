...
// Vertex data of a triangle, with a color for each vertex
...
int main()
{
    ...
    // Now we have 2 attributes, pos and color
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);

    // Now we can enable the vertex attribute (layout (location = 0) in the shader)
    glEnableVertexAttribArray(0);

    // Do the same for the second attribute (color)
    // Mind the last param!
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // The triangle is multicolor because of fragment interpolation
